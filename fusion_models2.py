...
class MultiStageFusionClassifier(nn.Module):
    def __init__(self, out_dim=1, backbone='resnet18', feature_dim=256, pair_fusion_dim=256):
        super().__init__()
        self.num_modalities = 3
        ...
        backbone_output_dim = self.backbones[0].num_features
        self.projs = nn.ModuleList([
            nn.Linear(backbone_output_dim, feature_dim)
            for _ in range(self.num_modalities)
        ])

        # Stage 1: Pairwise fusion
        self.pair1 = PairwiseFusion(feature_dim, pair_fusion_dim)
        self.pair2 = PairwiseFusion(feature_dim, pair_fusion_dim)

        # Stage 2: Fuse the pair outputs
        self.middle_fusion = PairwiseFusion(pair_fusion_dim, pair_fusion_dim)

        # Stage 3: Attention over pair + mid-level
        self.attn_fusion = AttentionFusion(embed_dim=pair_fusion_dim)

        # Final classifier
        self.classifier = nn.Sequential(
            nn.Linear(pair_fusion_dim, 128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, out_dim)
        )

    def forward(self, imgs):  # imgs: list of 4 tensors (B, 1, 256, 256)
        B = imgs[0].shape[0]
        device = imgs[0].device
        ...
        # Stage 1: pairwise fusion
        fused_12 = self.pair1(feats[0], feats[1])
        fused_34 = self.pair2(feats[0], feats[2])

        # Stage 2: middle fusion
        fused_mid = self.middle_fusion(fused_12, fused_34)

        # Stage 3: attention across [f12, f34, fmid]
        stack_all = torch.stack([fused_12, fused_34, fused_mid], dim=1)  # (B, 3, D)
        fused = self.attn_fusion(stack_all)
...
class BinaryClassificationTorch(nn.Module):
    def __init__(self, input_dim=64, output_size = 5, num_classes=1, radiomics=False, radiomics_dim=463,
        ...
        self.fusion_model = MultiStageFusionClassifier() #MultiModalCancerClassifierWithAttention()
...
if __name__ == "__main__":
    model = MultiStageFusionClassifier() #MultiModalCancerClassifierWithAttention()
