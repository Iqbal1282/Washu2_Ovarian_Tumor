...
class BinaryClassification(pl.LightningModule):
    ...
    def _common_step(self, batch, batch_idx):
        if len(batch) == 2: 
            ...
            loss = self.loss_fn(scores, y.float())*0.005 + self.loss_fn(scores_tail[0], y2.float())*0.1 + \
                        self.loss_fn(scores_tail[1], y2.float()) + self.loss_fn(scores_tail[2], y2.float()) + self.loss_fn(scores_tail[3], y2.float()) +\
                        self.loss_fn2(scores, y.float())*0.005 + self.loss_fn2(scores_tail[0], y2.float())*0.1 + \
    ...
    def forward(self, x, x2_radiomics=None):    
        ...
        x_center = self.center_encoder(x*(x_sdf<.4)) 
